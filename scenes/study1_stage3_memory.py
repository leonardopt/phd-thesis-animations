...
Render:
    ...
    uv run manim scenes/study1_stage3_memory.py Study1Stage3MemoryExpDesign -qh
"""
...
class Study1Stage3MemoryExpDesign(Scene):
    ...
    def construct(self) -> None:
        ...
        for (category, obj), row_y in zip(set_specs, row_y_positions):
            ...
            for col_idx, stim_idx in enumerate(selected_indices):
                ...
                if obj == "sofa":
                    dot = MathTex(r"\vdots", color=MGREY, font_size=24).next_to(card, DOWN, buff=0.10)
                    dot.set_z_index(5)
        ...
        probe1_idx = int(lake_row["distractor_2_position"])
    ...
    large: bool,
            ...
            image_height = 0.95 if large else 0.38
            fixation_height = 0.18 if large else 0.05
            ...
            if kind == "response":
                response_size = 18 if large else 7
                response_buff = 0.14 if large else 0.035
                ...
                # ── Arrow under "TWO" ─────────────────────────────
                arrow_length = 0.40 if large else 0.18
                arrow = Arrow(
                    ...
                    buff=0,
                    tip_length=0.3 if large else 0.1,
                )

                arrow.next_to(left_text, DOWN, buff=0.06 if large else 0.03)
        ...
        hold_before_park = 0.45
        park_run_time = 0.5
        hold_after_park = 0.12
        ...
        for idx, (duration, label, kind) in enumerate(stage_specs):
            ...
            next_content = stage_content(stage_specs[idx + 1][2], right_center, large=True)
            step_run_time = 0.5
            ...
            self.wait(0.30 if stage_specs[idx + 1][2] == "delay" else 0.18)
