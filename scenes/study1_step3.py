...
SIMILARITY_CSV_PATH = env_path("SIMILARITY_ANCHORS_FISH_CSV")
...
class _Study1Step3Base(Scene):
    ...
    def construct(self) -> None:
        ...
        def play_intro_and_matrix(keep_example_visible: bool = False) -> None:
            ...
            self.play(Write(repeat_note), run_time=0.30)
            fill_anims = [
                LaggedStart(
            ...
            self.play(*fill_anims, run_time=1.85)
            if not keep_example_visible:
                self.play(FadeOut(pair_card, pair_score, repeat_note), run_time=0.35)
